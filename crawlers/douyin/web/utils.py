...
def get_cookie_from_browser(domain: str = "douyin.com") -> dict:
    """
    从浏览器自动获取Cookie
    """
    ...
    for browser in browsers:
        try:
            ...
            cookie_dict = {c.name: c.value for c in cj if domain in c.domain}
            if cookie_dict:
                return cookie_dict
        except Exception:
            continue
...
class SecUserIdFetcher:
    ...
    async def get_sec_user_id(cls, url: str) -> str:
        ...
        try:
            ...
            proxy = TokenManager.proxies.get("http") or TokenManager.proxies.get(
                "https"
            )
            async with httpx.AsyncClient(
                transport=transport, proxy=proxy, timeout=10
            ) as client:
